...
us<-map_data('state')
ggplot(zipmap,aes(long,lat)) + geom_polygon(data=us,aes(x=long,y=lat,group=group),color='gray',fill=NA,alpha=.35)+geom_point(aes(size = count))+xlim(-125,-65)+ylim(20,50)+theme_bw()+theme(axis.text=element_text(size=18),
                                                                                                                                                                                            axis.title=element_text(size=18),plot.title = element_text(size=25, hjust=0.5),text = element_text(size = 18))+ggtitle("Survey Distribution")+xlab("Longitude") + ylab("Latitude")